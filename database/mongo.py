...
class MongoStore:
    def __init__(self) -> None:
        ...
        self.rows: Collection = self._db["rows"]
    ...
    def create_report(
        self, *, filename: str, header: Optional[str], date_range: Optional[str]
    ) -> str:
        doc = {
            "filename": filename,
            "header": header,
    ...
    def move_amount(
        self, row_id: str, *, from_bucket: str, to_bucket: str, amount: float
    ) -> bool:
    ...
    def reset_report(self, report_id: str) -> bool:
        ...
        if not ops:
            return False
        for op in ops:
            self.rows.update_one(op["filter"], op["update"])
        return True
